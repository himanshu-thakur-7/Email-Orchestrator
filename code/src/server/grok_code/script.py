...
EMAIL_DOMAINS = ["gmail.com", "yahoo.com", "outlook.com", "hotmail.com", "example.com", 
                "loanservice.com", "mortgagehelp.net", "financialaid.org"]
...
def generate_random_email():
    ...
    if random.random() > 0.7:
        ...
        second_part = ''.join(random.choices(string.ascii_lowercase + string.digits, k=second_part_length))
...
async def classify_email(email_body: str) -> dict:
    ...
    while run_response.status not in ["completed", "failed"]:
        run_response = await client.beta.threads.runs.retrieve(thread_id=thread_id, run_id=run_response.id)

    if run_response.status == "failed":
        raise HTTPException(status_code=500, detail="OpenAI Assistant failed to process request")
...
def search_similar_emails(email_text: str):
    ...
    return []  # Placeholder for vector search logic
...
async def get_documents(collection_name: str, limit: int = 100, skip: int = 0):
    ...
    try:
        ...
        documents = list(target_collection.find({}, {'_id': False}).skip(skip).limit(limit))
...
async def list_databases():
    ...
    try:
        ...
        filtered_databases = [db_name for db_name in databases 
...
@app.post("/configure")
async def configure_assistant(file: UploadFile = File(...), prompt_text: str = Form(...)):
    """
    ...
    try:
        ...
        assistant_response = create_assistant_with_vector_store(temp_file_path, prompt_text)
        ...
        raise HTTPException(status_code=500, detail=f"Error configuring assistant: {str(e)}")
